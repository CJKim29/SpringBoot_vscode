			...
			<div class="panel-body">
				<table class="table">
					...
					<tr>
						<th>회원등급</th>
						<td>
							<!-- 로그인 유저가 관리자라면 회원등급 수정 가능 -->
							<c:if test="${ user.mem_grade eq '관리자' }">
								<select style="width:50%" class="form-control" name="mem_grade">
									<option value="일반">일반</option>
									<option value="관리자">관리자</option>
								</select>
							</c:if>
							<!-- 로그인 유저가 일반유저라면 회원등급 수정 불가능 -->
							<c:if test="${ user.mem_grade eq '일반' }">
								<input style="width: 50%" class="form-control"
										name="mem_grade" value="${ vo.mem_grade }" readonly="readonly">
								</c:if>
						</td>
					</tr>
